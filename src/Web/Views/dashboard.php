      ...
      <div class="flex items-center">
        ...
        <div class="ml-4 w-0 flex-1">
          <dl>
            ...
            <dd class="text-2xl font-bold text-gray-900 dark:text-white/90">
              <?php
              $successRate = $runStats['total_runs'] >
                0 ? round(($runStats['completed_runs'] / $runStats['total_runs']) * 100, 1) : 0;
              echo
              $successRate; ?>%
            </dd>
          </dl>
          ...
          <?php if (empty($toolUsage)): ?>
            <div class="px-6 py-8 text-center">
              ...
              <p class="text-xs text-gray-400 dark:text-gray-500">
                Create agents with tools to see analytics
              </p>
            </div>
          <?php else: ?>
            <div class="p-6">
              <div class="space-y-4">
                ...
                foreach (
                  array_slice($toolUsage, 0, 5, true) as $tool =>
                  $usage
                ): $percentage = ($usage / $maxUsage) * 100;
                  $toolIcons = ['Math' => 'M9 7h6m0
              10v-3m-3+3.5h3m-6-6h.01M9 11h.01M11 11h.01M15 11h.01', 'Search' => 'M21 21l-6-6m2-5a7
              7 0 11-14 0 7 7 0 0114 0z', 'Weather' => 'M3 15a4 4 0 004 4h9a5 5 0 10-.1-9.999 5.002
              5.002 0 10-9.78 2.096A4.001 4.001 0 003 15z', 'ReadPDF' => 'M9 12h6m-6 4h6m2 5H7a2 2 0
              01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2
              2z'];
                  $iconPath = $toolIcons[$tool] ?? 'M10.325 4.317c.426-1.756 2.924-1.756 3.35
              0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065
              2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826
              3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724
              ...
              001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z'; ?>
                  <div class="flex items-center justify-between">
                    <div class="flex items-center gap-3">
                      <div class="flex-shrink-0">
                        <div
                          class="w-8 h-8 rounded-lg bg-brand-100 dark:bg-brand-900/20 flex items-center justify-center">
                          <svg
        ...
        <div class="border-t border-gray-100 dark:border-gray-800">
          <?php if (empty($recentThreads)): ?>
            <div class="px-6 py-12 text-center">
              ...
              <h3 class="mt-2 text-sm font-medium text-gray-900 dark:text-white/90">
                No conversations
              </h3>
              <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
      ...
      <div class="border-t border-gray-100 dark:border-gray-800 p-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div class="text-center">
            <div
              class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-blue-100 dark:bg-blue-900/20 mb-3">
              <svg
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">
              <?php
              $trendPercent = $threadStats['recent'] >
                0 ? '+' . round(($threadStats['recent'] / max($threadStats['total'] -
                  $threadStats['recent'], 1)) * 100) : '0';
              echo $trendPercent; ?>% increase this week
            </p>
            <p class="text-xs text-gray-600 dark:text-gray-400 mt-2">
              Your engagement is
              <?= $threadStats['recent'] >
                3 ? 'growing steadily' : 'just getting started' ?>
            </p>
          </div>
          ...
          <div class="text-center">
            <div
              class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-purple-100 dark:bg-purple-900/20 mb-3">
              <svg
            ...
            <p class="text-xs text-gray-600 dark:text-gray-400 mt-2">
              ...
              <?php elseif (
                $agentStats['with_tools'] / $agentStats['total'] >
                0.5
          ...
          <div class="text-center">
            <div
              class="inline-flex items-center justify-center w-12 h-12 rounded-full bg-green-100 dark:bg-green-900/20 mb-3">
              <svg
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    const performanceData = {
      ...
      datasets: [{
        data: [
          completedRuns,
          failedRuns,
          Math.max(0, totalRuns - completedRuns - failedRuns)
        ],
        backgroundColor: [
          chartColors.success,
          chartColors.error,
    ...
    document.addEventListener('alpine:updated', function() {
      // This would ideally listen for dark mode changes and update chart colors
      // For now, charts will update on page refresh
    });
  });

  // Utility function to refresh charts (called when period changes)
  function refreshCharts(period) {
    ...
    // You could make an AJAX call here to get fresh data
